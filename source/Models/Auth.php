...
class Auth extends Model
{
     ...
     * @return bool
     */
    public function register(User $user, Responsa $responsa): bool
    {
        $resp = (new Responsa())->findByAluno($user->pes_nome);



        if (!$resp) {

            $this->message->warning("O Aluno informado não está cadastrado.");
            return false;
        }

        $responsa = (new Responsa())->findByResponsa($user->resp_nome);



        if (!$responsa) {

            $this->message->warning("O Responsável informado não confere com os dados do sistema.");
            return false;
        }
        ...
        $message = $view->render("confirm", [
            "resp_nome" => $user->resp_nome,
            "pes_nome" => $user->pes_nome,
            "email"=> $user->email,
        ...
        (new Email())->bootstrap(
            "Responsável Atualizado " . CONF_SITE_NAME,
            $message,
            CONF_MAIL_USER,
            "{$user->first_name} {$user->last_name}"
        )->send();

        $responsa->confirmado = 1;
        $responsa->save();
        return true;
    }
