    ...
    <div class="row">
        <div class="col-md-12">
            <table class="table table-bordered table-hover">
                ...
                $sql = $conn->query("select pes, resp, telefone1, telefone2, email from dolistcard left join users on (dolistcard.pes = users.pes_nome) where telefone1 is null group by pes");
                while($data = $sql->fetch_array()) {
                    echo '
                                    <tr>
                                        
                                        
                                        <td>'.utf8_encode($data['pes']).'</td>
                                        <td>'.utf8_encode($data['resp']).'</td>
